...
def get_status_code(url):
	try:
		...
		if len(r.history) > 0:
			...
			tags2 = soup.findAll("h2", {"id": "header-phone"})
			tags3 = soup.findAll("img")
			tags4 = soup.findAll("h1")
			#clean = tags4.text.strip()
			#print(titletag);
			#print(tags2);
			#print(tags3);
			print(tags4);
			...
			return str(code) + '\t' + str(len(r.history)) + '\t' + chain + '\t' + final_url + '\t' + url_ip + '\t' + url_ns + '\t' + titletag + '\t' + str(tags3)
		else:
			final_url = r.url
			list = tldextract.extract(final_url)
			final_domain = list.domain + '.' + list.suffix
			...
			return str(r.status_code) + '\t\t\t' + final_url + '\t' + url_ip + '\t' + url_ns + '\t' + titletag
...
with open(output_file, 'w') as o_file:
	o_file.write('URL\tStatus\tNumber of redirects\tRedirect Chain\tFinal URL\tFinal URL IP\tFinal URL NS\tTitle Tag\t\n')
	f = open(input_file, "r")
