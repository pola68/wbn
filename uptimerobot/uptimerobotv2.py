...
csv_file.write("Monitor Name,URL,Ratios,Event,Date,Duration (sec),Details\n")
...
def send_request(offset):
	...
	for monitor in monitory:
		...
		#print(monitor["custom_uptime_ratio"])
		logs = (monitor["logs"])
		...
		for log in logs:
			...
			file_content += monitor["friendly_name"] + "," + monitor["url"] + "," + monitor["custom_uptime_ratio"] + "," + str(log_type) + "," + dtime + "," + str(duration) + "," + reason_detail + "\n"
...
oneDayAgo = today - datetime.timedelta(days=1)
...
oneDayEvents = headers
sevenDayEvents = headers

with open('uptime-output.csv') as csvfile:
...
#1 Day Metrics
email_summary = "LAST DAY REVIEW:\n"
email_summary += "Last Day Up Minutes: %d" % oneDayTotalUp + "\n"
email_summary += "Last Day Down Minutes: %d" % oneDayTotalDown + "\n"

oneDayAllTime = oneDayTotalUp + oneDayTotalDown
oneDayUpRatio = (oneDayTotalUp / oneDayAllTime) * 100
email_summary += "Last Day Uptime Ratio: " + str(round(oneDayUpRatio,2)) + "%\n\n\n"
...
email_summary += "OUTAGES THAT HAPPENED WITHIN LAST DAY:\n\n\n"
